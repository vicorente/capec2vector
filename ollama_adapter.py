...
KALI_API_BASE_URL = os.environ.get(
    "KALI_API_BASE_URL", f"http://172.16.11.111:{API_PORT}"
)
    ...
    def __init__(self, model_name: str = DEFAULT_MODEL):
        ...
        try:
            ...
            if health_response.status_code == 200:
                ...
                logger.error(
                    f"API server health check failed: {health_response.status_code}"
                )
...
"""
    ...
    def chat(self):
        ...
        print(
            "You can directly ask to run security tools like: 'Run nmap on 10.10.10.10'\n"
        )
            ...
            try:
                ...
                if tool_result:
                    ...
                    if tool_request and "tool" in tool_request:
                        ...
                        confirm = input(
                            f"Run {tool_name} with params {params}? (y/n): "
                        )
                        if confirm.lower() in ["y", "yes"]:
                            ...
                            if result["status"] == "success":
                                ...
                                print(
                                    f"Failed to execute {tool_name}: {result.get('message', 'Unknown error')}"
                                )
...
class OllamaAPI:
    def __init__(self, host="0.0.0.0", port=5050):
        ...
        self.app.route("/execute_tool", methods=["POST"])(self.execute_tool_endpoint)
    ...
    def chat_endpoint(self):
        try:
            ...
            response = self.adapter.client.chat(
    ...
    def list_tools(self):
        return jsonify(
            {"tools": list(KALI_TOOLS.keys()), "info": self.adapter.tools_info}
        )

    def execute_tool_endpoint(self):
        try:
            data = request.get_json()
            if not data or "tool" not in data or "params" not in data:
                return jsonify({"error": "Missing tool or params in request"}), 400

            result = self.adapter.execute_tool(data["tool"], data["params"])
    ...
    def health_check(self):
        return jsonify({"status": "healthy", "model": self.adapter.model_name})
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run the Ollama Adapter for Kali Linux Tools"
    )
    parser.add_argument(
        "--api-port",
        type=int,
    ...
    parser.add_argument(
        "--api-mode",
        action="store_true",
        help="Run in API mode instead of interactive mode",
    )
    parser.add_argument(
        "--api-host",
        type=str,
...
if __name__ == "__main__":
    ...
    try:
        if args.api_mode:
            logger.info(
                f"Starting API server on {args.api_host}:{args.api_server_port}"
            )
            api = OllamaAPI(host=args.api_host, port=args.api_server_port)
            api.run()
        else:
